...
import type { WorkflowValidationError } from "@/hooks/use-workflow-editor"
...
{/* Run button — disabled with tooltip when validation errors exist */}
<TooltipProvider>
<Tooltip>
...
<Button
size="sm"
onClick={hasErrors ? undefined : onRun}
disabled={hasErrors}
className="gap-1.5"
aria-describedby={hasErrors ? "run-tooltip" : undefined}
>
{hasErrors ? (
...
{hasErrors && (
<TooltipContent id="run-tooltip" side="bottom" className="max-w-xs">
<ul className="space-y-0.5">
{validationErrors.map((e, i) => (
<li key={i} className="flex items-start gap-1">
<AlertCircleIcon className="size-3 shrink-0 mt-0.5 text-destructive" />
{e.message}
</li>
))}
</ul>
