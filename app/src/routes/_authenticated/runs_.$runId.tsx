		...
		<Collapsible open={expanded} onOpenChange={setExpanded}>
			<div className="rounded-lg border bg-card overflow-hidden">
				<div className="flex items-center gap-2 px-3 py-2 border-b bg-muted/30">
					...
					<span className="text-xs font-medium flex-1 font-mono truncate">{step.stepName}</span>
...
function stepStatusSymbol(status: string): string {
...
function buildTerminalLines(steps: RunStep[]): TerminalLine[] {
	const lines: TerminalLine[] = []
	for (const step of steps) {
		if (step.status === "pending") continue
		const ts = step.startedAt
			? new Date(step.startedAt).toISOString().replace("T", " ").slice(0, 23)
			: "—"
		lines.push({
			ts,
			stepName: step.stepName,
			status: step.status,
			message: step.logs?.split("\n")[0] ?? `[${step.status}]`,
		})
		// Extra log lines after the first
		if (step.logs) {
			const extra = step.logs.split("\n").slice(1)
			for (const line of extra) {
				if (line.trim()) {
					lines.push({ ts: "", stepName: "", status: step.status, message: line })
				}
			}
		}
	}
...
function TerminalView({ steps }: { steps: RunStep[] }) {
	const bottomRef = useRef<HTMLDivElement>(null)
	const lines = buildTerminalLines(steps)
	...
	return (
		<div className="rounded-lg border bg-[#0d1117] text-[#e6edf3] font-mono text-xs overflow-hidden">
			<div className="flex items-center gap-2 px-3 py-1.5 bg-[#161b22] border-b border-[#30363d]">
				<TerminalIcon className="size-3 text-[#8b949e]" />
				<span className="text-[#8b949e] text-[10px]">Execution log</span>
			</div>
			<div className="p-3 space-y-0.5 max-h-96 overflow-y-auto">
				{lines.length === 0 ? (
					<span className="text-[#8b949e]">Waiting for steps…</span>
				) : (
					lines.map((line, i) => (
						<div key={i} className="flex gap-2 leading-5">
							{/* Timestamp column — fixed width so log lines stay aligned */}
							<span className="text-[#8b949e] shrink-0 tabular-nums w-[11ch]">
								{line.ts || ""}
							</span>
							{/* Status symbol + step name — hidden for continuation lines */}
							{line.stepName ? (
								<>
									<span className={`shrink-0 w-[1ch] ${stepStatusColor(line.status)}`}>
										{stepStatusSymbol(line.status)}
									</span>
									<span className="text-[#79c0ff] shrink-0 truncate max-w-[180px]">
										{line.stepName}
									</span>
								</>
							) : (
							...
							<span className="text-[#e6edf3] break-all">{line.message}</span>
						</div>
					))
				)}
				<div ref={bottomRef} />
...
function RunDetailPage() {
	...
	const [view, setView] = useState<"steps" | "terminal">("terminal")
